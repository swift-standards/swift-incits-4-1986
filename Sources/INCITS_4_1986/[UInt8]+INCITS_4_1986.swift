...
extension [UInt8] {
    ...
    /// let lf = [UInt8].ascii(lineEnding: .lf)  // [0x0A]
    /// ```
    ///
    /// ## See Also
    ///
...
// MARK: - String to Byte Array Conversion
...
extension [UInt8] {
    ...
    /// ```
    ///
    /// - Parameter string: The string to convert to ASCII bytes
    ...
    /// let unix = [UInt8].ascii(lineEnding: .lf)      // [0x0A]
    /// let mac = [UInt8].ascii(lineEnding: .cr)       // [0x0D]
    /// let windows = [UInt8].ascii(lineEnding: .crlf) // [0x0D, 0x0A]
    ///
    /// // Build byte arrays with line endings
    /// var bytes: [UInt8] = [UInt8].ascii("Line 1")!
    /// bytes += [UInt8].ascii(lineEnding: .crlf)
    /// bytes += [UInt8].ascii("Line 2")!
    /// ```
    ///
    /// - Parameter lineEnding: The line ending style to convert to bytes
