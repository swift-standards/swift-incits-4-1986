...
@Suite
struct `Edge Cases - ASCII Boundaries` {
...
struct `Edge Cases - Case Conversion` {
    ...
    func `already uppercase string unchanged`() {
        ...
        #expect(bytes.ascii.convertingCase(to: .upper) == bytes)
    }

    @Test
    func `already lowercase string unchanged`() {
        let bytes: [UInt8] = [UInt8.ascii.h, .ascii.e, .ascii.l, .ascii.l, .ascii.o]
        #expect(bytes.ascii.convertingCase(to: .lower) == bytes)
    }
    ...
    func `symbols unchanged during case conversion`() {
        ...
        #expect(symbols.ascii.convertingCase(to: .upper) == symbols)
        #expect(symbols.ascii.convertingCase(to: .lower) == symbols)
    }

    @Test
    func `empty array case conversion`() {
        let empty: [UInt8] = []
        #expect(empty.ascii.convertingCase(to: .upper) == [])
        #expect(empty.ascii.convertingCase(to: .lower) == [])
    }
    ...
    func `case conversion at letter boundaries`() {
        ...
        #expect(nonLetters.ascii.convertingCase(to: .upper) == nonLetters)
        #expect(nonLetters.ascii.convertingCase(to: .lower) == nonLetters)
    }
...
struct `Edge Cases - Constants` {
    ...
    func `crlf constant is exactly two bytes`() {
        #expect(INCITS_4_1986.crlf.count == 2)
        #expect(INCITS_4_1986.crlf == [UInt8.ascii.cr, UInt8.ascii.lf])
    }

    @Test
    func `crlf order is CR then LF`() {
        #expect(INCITS_4_1986.crlf[0] == UInt8.ascii.cr)
        #expect(INCITS_4_1986.crlf[1] == UInt8.ascii.lf)
    }
}
