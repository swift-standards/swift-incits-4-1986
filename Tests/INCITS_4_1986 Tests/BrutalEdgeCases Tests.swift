...
struct `Brutal Tests` {
    ...
    struct `Brutal - Every Single Byte Value` {
        ...
        func `case conversion involution for every byte`(byte: UInt8) {
            ...
            if byte.ascii.isLetter {
                ...
                #expect(cycle == byte.ascii.ascii(case: .upper), "Case involution failed for 0x\(String(byte, radix: 16))")
            }
        }
    }
    ...
    struct `Brutal - Predicate Consistency` {
        ...
        func `every ASCII byte is either control or printable`(byte: UInt8) {
            ...
            #expect(isControl != isPrintable, "Byte 0x\(String(byte, radix: 16)) must be exactly one of control or printable")
        }
    ...
    struct `Brutal - Unicode Confusion` {
        ...
        func `UTF-16 surrogate range bytes rejected`() {
            ...
            for byte in surrogateRangeBytes {
                #expect(![byte].ascii.isAllASCII, "Surrogate-range byte 0x\(String(byte, radix: 16)) should be rejected")
            }
        }
    ...
    struct `Brutal - Trimming Pathological Cases` {
        ...
        func `10000 leading spaces`() {
            let massive = String(repeating: " ", count: 10_000) + "content"
        ...
        func `10000 trailing spaces`() {
            let massive = "content" + String(repeating: " ", count: 10_000)
            let trimmed = massive.trimming(.ascii.whitespaces)
        ...
        func `trim when every character is in set`() {
            ...
            #expect(trimmed == "", "Should trim to empty when all match")
        }
        
        @Test
        func `trim single character strings`() {
            #expect(" ".trimming(.ascii.whitespaces) == "")
    ...
    struct `Brutal - Case Conversion Torture` {
        ...
        func `case conversion with every non-letter byte`() {
            ...
            for byte in nonLetters {
                #expect(byte.ascii.ascii(case: .upper) == byte, "Non-letter 0x\(String(byte, radix: 16)) should be unchanged by uppercase")
                #expect(byte.ascii.ascii(case: .lower) == byte, "Non-letter 0x\(String(byte, radix: 16)) should be unchanged by lowercase")
            }
        }
    ...
    struct `Brutal - Validation Exhaustive` {
        ...
        func `validation with every possible first byte`() {
            for firstByte in UInt8(0)...UInt8(255) {
                ...
                #expect(array.ascii.isAllASCII == expected, "First byte 0x\(String(firstByte, radix: 16)) validation incorrect")
            }
        }
        ...
        func `validation with every possible last byte`() {
            for lastByte in UInt8(0)...UInt8(255) {
                ...
                #expect(array.ascii.isAllASCII == expected, "Last byte 0x\(String(lastByte, radix: 16)) validation incorrect")
            }
        }
    }
    ...
    struct `Brutal - String Conversion Edge Cases` {
        ...
        func `string conversion fails for non-ASCII`() {
            for byte in UInt8(128)...UInt8(255) {
                #expect(String.ascii([byte]) == nil, "Non-ASCII byte 0x\(String(byte, radix: 16)) should fail conversion")
            }
        }
...
extension `Performance Tests` {
    ...
    struct `Brutal - Performance Torture` {
        ...
        @Test(.timed(threshold: .milliseconds(3000), maxAllocations: 1_000_000))
